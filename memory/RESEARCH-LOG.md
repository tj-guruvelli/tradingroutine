...
## 2026-08-18 — Setup Scan (18:38 ET, cloud)

Full-universe scan (60 tickers checked from `config/rules.json`
watchlist_tiers.immediate, via `scripts/setup-scan-cloud.mjs` against Alpaca
bars/quotes — MCP unavailable in cloud). **0 grade-A hits, 2 grade-B hits.**
0 errors. Post-close run — figures unchanged from the 16:38 ET scan (market
closed at 16:00 ET). No Telegram/ClickUp notify sent per routine rule (0
grade-A hits).
...
### Setup Scan (18:39 ET, cloud)
Full-universe scan, 60 candidates checked, 0 errors. 0 grade-A hits, 4 grade-B.
Same four names as the 16:39 ET run, unchanged setups.
...
## 2026-08-24 — Setup Scan (16:39 ET, cloud)
...
## 2026-08-25 — Setup Scan (18:38 ET, cloud)

Full-universe scan (60 tickers checked from `config/rules.json`
watchlist_tiers.immediate, via `scripts/setup-scan-cloud.mjs` against Alpaca
bars/quotes — MCP unavailable in cloud). **0 grade-A hits, 1 grade-B hit.**
0 errors. No Telegram/ClickUp notify sent per routine rule (0 grade-A hits).
Same single hit as the 16:38 ET run this session (BMNR), stronger RSI/ADX —
no new tickers rotated in.
...
research (Step 6 n/a — no positions). No action taken -> no ClickUp
notification (Step 7 skip per "only if action was taken"). 38 trading days
since launch (Jul 9) with zero entries. Weekly trade count: 0/3 (new week
of Aug 31). Equity flat at $100,000 — confirmed-live-vs-$10k-baseline
mismatch flagged Jul 27 remains unresolved, 48th straight session, operator
...
`node scripts/setup-scan-cloud.mjs` (Alpaca bars/quotes, cloud variant —
no `stock_score` gate on Setup B, see routine note). 0 grade-A hits, 0
grade-B hits — no setups triggered this run (second scan today, same
result as 16:38 ET run).
...
Re-scan post-open (11:15 ET, ~1h45m after 9:30 open). Raw scan returned two
rows: KLIC +7.24% (prev_close $78.735 -> $84.435) and TRMD +7.08%
(prev_close $33.06 -> $35.40), both clearing the 5% gap and $3 price
floors. Both excluded on the volume gate: `volume` field = 6,154 (KLIC)
and 16,709 (TRMD), well under the 50,000 floor. Same known data bug as the
10:17 ET run and the 08-31 10:18 ET run (5th occurrence): the script's
`volume` field is today's forming daily-bar volume, not true premarket
volume — post-open it's mechanically low for a while after the open
regardless of real activity, so it under-reports and wrongly gates out
otherwise-qualifying gappers. 0 of 2 raw hits cleared all three filters.
No quick-scan table, no deep dive. `data/premarket_gappers_2026-09-02.json`
overwritten with the newer `scanned_at` timestamp, `gappers: []` unchanged.
...
errors. 0 grade-A hits, 3 grade-B hits (Momentum confluence: ADX14 > 20 AND
EMA9 > EMA21 — stock_score gate dropped per cloud-variant deviation). Same
three tickers as the 16:38 ET run this session — market closed since 16:00
ET, so daily bars are unchanged.
...
No grade-A hits — no Telegram/ClickUp notification sent (matches the ">= 1
...
## 2026-09-04 — Setup Scan (18:38 ET, cloud)

Full-universe scan (60 tickers checked from `config/rules.json`
watchlist_tiers.immediate, via `scripts/setup-scan-cloud.mjs` against Alpaca
bars/quotes — MCP unavailable in cloud). **0 grade-A hits, 5 grade-B hits.**
0 errors. No Telegram/ClickUp notify sent per routine rule (0 grade-A hits).
Same 5 tickers and near-identical readings as the 16:38 ET run — both scans
ran after the 16:00 ET close, so the same latest daily bar backs both.
...
watchlist_tiers.immediate, via `scripts/setup-scan-cloud.mjs` against Alpaca
bars/quotes — MCP unavailable in cloud). **0 grade-A hits, 2 grade-B hits**:
same pair as the 16:38 ET run — HAFN and BWLP still clear Setup B (ADX14 > 20
AND EMA9 > EMA21). Setup A (TJL breakout) not checkable — `grade_a_possible:
false` (`setup_a_skipped_reason: "outside 10:00-15:30 ET"`), this is the
18:30 ET Chicago cron fire, after the 15:30 ET intraday-breakout cutoff. No
grade-A hit, so no Telegram/ClickUp notify sent per routine rule.
...
## 2026-09-09 — Setup Scan (18:40 ET, cloud)

Full-universe scan (60 tickers checked from `config/rules.json`
watchlist_tiers.immediate, via `scripts/setup-scan-cloud.mjs` against Alpaca
bars/quotes — MCP unavailable in cloud). **0 grade-A hits, 4 grade-B hits**:
same four tickers as the 16:38 ET run (HAFN, BWLP, TRMD, CMBT), with
identical indicator values — expected, since both runs read the same
daily bar (market closed at 16:00 ET, no new daily close between runs).
Setup A (TJL breakout) not checkable for any ticker this run —
`setup_a_checkable: false` across the board, consistent with
`grade_a_possible: false` (`setup_a_skipped_reason: "outside 10:00-15:30
ET"`) — this is the 16:30 ET Chicago cron fire (18:40 ET wall time), after
the 15:30 ET intraday-breakout cutoff. No grade-A hit, so no
Telegram/ClickUp notify sent per routine rule.

### Setup Scan (18:40 ET, cloud)
| TICKER | GRADE | SETUP(S) | TIMEFRAME | TRIGGER |
| ------ | ----- | -------- | --------- | ------- |
| HAFN | B | Momentum confluence | daily swing | ADX 23.3, EMA9 $8.70 > EMA21 $8.30, RSI 67.6, px $8.91 vs SMA200 $7.22 |
| BWLP | B | Momentum confluence | daily swing | ADX 22.4, EMA9 $24.20 > EMA21 $23.60, RSI 55.6, px $19.00 vs SMA200 $17.96 |
...
| TICKER | GRADE | SETUP(S)             | TIMEFRAME   | TRIGGER |
| ------ | ----- | -------------------- | ----------- | ------- |
| T      | B     | Momentum confluence  | daily swing | ADX 32.03, EMA9 25.61 > EMA21 25.27 |
| TRMD   | B     | Momentum confluence  | daily swing | ADX 22.28, EMA9 34.05 > EMA21 32.64 |

No grade-A hits (Setup A not evaluated this run — 16:38 ET fire lands after the 15:30 ET cutoff). No Telegram alert sent (0 grade-A). Candidates persisted to `data/setup-scan_cloud_2026-09-11_1638ET.json` for `market-open.md` to pick up next session.
...
watchlist_tiers.immediate, via `scripts/setup-scan-cloud.mjs` against Alpaca
bars/quotes — MCP unavailable in cloud). **0 grade-A hits, 3 grade-B hits**:
HAFN, BMNR, and BLSH all clear Setup B (ADX14 > 20 AND EMA9 > EMA21; note the
local `/setup-scan` stock_score>=6 gate is dropped here, MCP-only). Setup A
(TJL breakout) not checkable for any ticker this run — `setup_a_checkable:
...
## 2026-09-15 — Setup Scan (18:38 ET, cloud)
...
## 2026-09-16 — Setup Scan (18:38 ET, cloud)
...
## 2026-09-17 — Setup Scan (16:39 ET, cloud)

Full-universe scan via `node scripts/setup-scan-cloud.mjs` (60 candidates,
`config/rules.json` watchlist_tiers.immediate). `grade_a_possible: false` —
16:39 ET is outside the 10:00-15:30 ET intraday-breakout window, so Setup A
was not evaluated for any symbol this run (grade A unreachable). 1 hit
(grade B), 0 errors.
...
grade B/C only. 1 hit, 0 errors (same single hit as the 16:33 ET run this
session — no change in setup state over the two hours).
